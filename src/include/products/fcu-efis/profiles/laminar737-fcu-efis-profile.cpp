...
Laminar737FCUEfisProfile::Laminar737FCUEfisProfile(ProductFCUEfis *product) :
    ...
    debug_force("737 Profile: Constructor called\n");
    
    // Monitor display datarefs to ensure they trigger updates
    Dataref::getInstance()->monitorExistingDataref<float>("sim/cockpit2/autopilot/airspeed_dial_kts_mach", [](float value) {
        debug("737 Profile: Airspeed changed: %.2f\n", value);
    });
    
    Dataref::getInstance()->monitorExistingDataref<int>("sim/cockpit/autopilot/airspeed_is_mach", [](int value) {
        debug("737 Profile: Mach mode changed: %d\n", value);
    });
    
    Dataref::getInstance()->monitorExistingDataref<float>("sim/cockpit/autopilot/heading_mag", [](float value) {
        debug("737 Profile: Heading changed: %.1f\n", value);
    });
    
    Dataref::getInstance()->monitorExistingDataref<float>("sim/cockpit/autopilot/altitude", [](float value) {
        debug("737 Profile: Altitude changed: %.0f\n", value);
    });
    
    Dataref::getInstance()->monitorExistingDataref<float>("sim/cockpit/autopilot/vertical_velocity", [](float value) {
        debug("737 Profile: V/S changed: %.0f\n", value);
    });
    
    Dataref::getInstance()->monitorExistingDataref<float>("sim/cockpit2/gauges/actuators/barometer_setting_in_hg_pilot", [](float value) {
        debug("737 Profile: Baro pilot changed: %.2f\n", value);
    });
    
    Dataref::getInstance()->monitorExistingDataref<float>("sim/cockpit2/gauges/actuators/barometer_setting_in_hg_copilot", [](float value) {
        debug("737 Profile: Baro FO changed: %.2f\n", value);
    });
    ...
    Dataref::getInstance()->monitorExistingDataref<std::vector<float>>("sim/cockpit2/electrical/instrument_brightness_ratio", [product](std::vector<float> brightness) {
        debug("737 Profile: Brightness changed, size=%lu\n", brightness.size());
        if (brightness.size() < 15) {
            debug("737 Profile: Brightness vector too small\n");
            return;
        }
        bool hasPower = Dataref::getInstance()->get<bool>("sim/cockpit/electrical/battery_on");
        debug("737 Profile: hasPower=%d, brightness[14]=%.3f\n", hasPower, brightness[14]);

        // Use appropriate brightness index for 737 instruments
        uint8_t target = hasPower ? brightness[14] * 255.0f : 0;
        debug("737 Profile: Setting LED brightness target=%d\n", target);
        ...
        debug("737 Profile: Screen brightness=%d\n", screenBrightness);
    ...
    Dataref::getInstance()->monitorExistingDataref<bool>("sim/cockpit/electrical/battery_on", [](bool poweredOn) {
        debug("737 Profile: Battery power changed: %d\n", poweredOn);
    ...
    Dataref::getInstance()->monitorExistingDataref<int>("laminar/B738/autopilot/cmd_a_status", [product](int status) {
        debug("737 Profile: AP1 (CMD A) status changed: %d\n", status);
    ...
    Dataref::getInstance()->monitorExistingDataref<int>("laminar/B738/autopilot/cmd_b_status", [product](int status) {
        debug("737 Profile: AP2 (CMD B) status changed: %d\n", status);
        // Status 2 = engaged and active
    ...
    Dataref::getInstance()->monitorExistingDataref<int>("sim/cockpit2/autopilot/autothrottle_arm", [product](int armed) {
        debug("737 Profile: A/THR armed changed: %d\n", armed);
    ...
    Dataref::getInstance()->monitorExistingDataref<int>("sim/cockpit2/autopilot/approach_status", [product](int status) {
        debug("737 Profile: APPR status changed: %d\n", status);
    ...
    Dataref::getInstance()->monitorExistingDataref<int>("sim/cockpit/autopilot/autopilot_state", [product](int state) {
        ...
        debug("737 Profile: AP state changed: %d, LOC active: %d\n", state, locActive);
    ...
    Dataref::getInstance()->monitorExistingDataref<int>("sim/cockpit2/EFIS/EFIS_weather_on_copilot", [product](int on) {
        debug("737 Profile: EFIS Right WX changed: %d\n", on);
    ...
    Dataref::getInstance()->monitorExistingDataref<int>("sim/cockpit2/EFIS/EFIS_tcas_on_copilot", [product](int on) {
        debug("737 Profile: EFIS Right TCAS changed: %d\n", on);
    ...
    Dataref::getInstance()->monitorExistingDataref<int>("sim/cockpit2/EFIS/EFIS_weather_on_pilot", [product](int on) {
        debug("737 Profile: EFIS Left WX changed: %d\n", on);
    ...
    Dataref::getInstance()->monitorExistingDataref<int>("sim/cockpit2/EFIS/EFIS_tcas_on_pilot", [product](int on) {
        debug("737 Profile: EFIS Left TCAS changed: %d\n", on);
        product->setLedBrightness(FCUEfisLed::EFISL_ARPT_GREEN, on ? 255 : 0);
    });
    
    debug_force("737 Profile: Constructor completed\n");
}

Laminar737FCUEfisProfile::~Laminar737FCUEfisProfile() {
    debug_force("737 Profile: Destructor called\n");
    // Unbind all datarefs
    ...
    debug_force("737 Profile: Destructor completed\n");
}
...
bool Laminar737FCUEfisProfile::IsEligible() {
    ...
    debug_force("737 Profile: IsEligible check - ICAO='%s'\n", icao.c_str());
    
    // Check for 737 variants (B731, B732, B733, B734, B735, B736, B737, B738, B739, etc.)
    if (icao.starts_with("B73")) {
        debug_force("737 Profile: IsEligible=TRUE (matched B73* pattern)\n");
        return true;
    }
    
    debug_force("737 Profile: IsEligible=FALSE\n");
    return false;
}
...
const std::vector<std::string> &Laminar737FCUEfisProfile::displayDatarefs() const {
    ...
    // debug_force("737 Profile: displayDatarefs returning %lu datarefs\n", datarefs.size());
    
    return datarefs;
}

const std::vector<FCUEfisButtonDef> &Laminar737FCUEfisProfile::buttonDefs() const {
...
void Laminar737FCUEfisProfile::updateDisplayData(FCUDisplayData &data) {
    debug("737 Profile: updateDisplayData called\n");
    
    bool hasPower = Dataref::getInstance()->get<bool>("sim/cockpit/electrical/battery_on");
    debug("737 Profile: hasPower=%d\n", hasPower);
    
    if (!hasPower) {
        debug("737 Profile: No power - clearing displays\n");
    ...
    // Speed display
    bool isMach = Dataref::getInstance()->get<int>("sim/cockpit/autopilot/airspeed_is_mach");
    float speed = Dataref::getInstance()->get<float>("sim/cockpit2/autopilot/airspeed_dial_kts_mach");
    debug("737 Profile: speed=%.2f, isMach=%d\n", speed, isMach);
    ...
    if (isMach) {
        ...
        data.speed = {machStr, {}};
        data.spdMach = true;
        debug("737 Profile: Speed display (MACH): '%s'\n", machStr.c_str());
    } else {
        // Display as knots
        std::ostringstream ss;
        ...
        data.speed = {ss.str(), {}};
        data.spdMach = false;
        debug("737 Profile: Speed display (KTS): '%s'\n", ss.str().c_str());
    }
    ...
    debug("737 Profile: Heading: %.1f -> '%s'\n", heading, headingSs.str().c_str());
    ...
    debug("737 Profile: Altitude: %.0f -> '%s'\n", altitude, altSs.str().c_str());
    ...
    data.vsSign = (vs >= 0);
    data.vsMode = true;
    debug("737 Profile: V/S: %.0f -> '%s' (sign=%d)\n", vs, vsSs.str().c_str(), data.vsSign);
    
    // Barometer displays (inHg)
    float baroPilot = Dataref::getInstance()->get<float>("sim/cockpit2/gauges/actuators/barometer_setting_in_hg_pilot");
    float baroFO = Dataref::getInstance()->get<float>("sim/cockpit2/gauges/actuators/barometer_setting_in_hg_copilot");
    debug("737 Profile: Baro Pilot=%.2f, FO=%.2f\n", baroPilot, baroFO);
    
    data.efisLeft.setBaro(baroPilot, true);
    data.efisRight.setBaro(baroFO, true);
    
    debug("737 Profile: updateDisplayData completed\n");
}

void Laminar737FCUEfisProfile::buttonPressed(const FCUEfisButtonDef *button, XPLMCommandPhase phase) {
    debug("737 Profile: buttonPressed called id=%d, phase=%d\n", button ? button->id : -1, phase);
    if (!button || button->dataref.empty() || phase == xplm_CommandContinue) {
        return;
    }

    debug("737 Profile: Button pressed - id=%d, name='%s', dataref='%s', type=%d, phase=%d, value=%.2f\n",
          button->id, button->name.c_str(), button->dataref.c_str(), 
          (int)button->datarefType, phase, button->value);

    auto datarefManager = Dataref::getInstance();

    // Special handling for A/THR button (id=5)
    if (button->id == 5 && phase == xplm_CommandBegin) {
        int armed = datarefManager->get<int>("sim/cockpit2/autopilot/autothrottle_arm");
        debug("737 Profile: A/THR current armed state: %d\n", armed);
        
        if (armed == 1) {
            // If armed, disable it by executing hard_off command
            debug("737 Profile: A/THR - disabling via hard_off command\n");
            datarefManager->executeCommand("sim/autopilot/autothrottle_hard_off");
        } else {
            // If not armed, arm it
            debug("737 Profile: A/THR - arming via command\n");
            datarefManager->executeCommand("sim/autopilot/autothrottle_arm");
        }
        debug("737 Profile: A/THR button handled\n");
        return;
    }
    ...
    if (button->datarefType == FCUEfisDatarefType::TOGGLE_VALUE) {
        if (phase == xplm_CommandBegin) {
            ...
            debug("737 Profile: Toggling dataref '%s' from %d to %d\n", 
        ...
        if (phase == xplm_CommandBegin) {
            debug("737 Profile: Setting dataref '%s' to %.2f\n", button->dataref.c_str(), button->value);
            datarefManager->set<float>(button->dataref.c_str(), button->value);
        }
    } else if (button->datarefType == FCUEfisDatarefType::BAROMETER_PILOT || button->datarefType == FCUEfisDatarefType::BAROMETER_FO) {
        if (phase == xplm_CommandBegin) {
            ...
            debug("737 Profile: Adjusting baro %s from %.2f to %.2f (increase=%d)\n",
                  isCaptain ? "PILOT" : "FO", baroValue, newBaro, increase);
            datarefManager->set<float>(datarefName, newBaro);
        }
    } else {
        // Default: execute as command (for EXECUTE_CMD_ONCE or unspecified type)
        debug("737 Profile: Executing command: '%s'\n", button->dataref.c_str());
        datarefManager->executeCommand(button->dataref.c_str(), phase);
    }
    
    debug("737 Profile: Button press handled\n");
}
