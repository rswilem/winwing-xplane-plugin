...
void ProductPAP3MCP::toggleShowLcdLabels() {
    ...
    if (g_pap3MenuId != nullptr && g_pap3LabelsIndex >= 0) {
        XPLMCheckMenuItem(g_pap3MenuId, g_pap3LabelsIndex, g_showLabels ? xplm_Menu_Checked : xplm_Menu_Unchecked);
    }
...
void ProductPAP3MCP::createSubmenuIfNeeded() {
    ...
    g_pap3MenuId = XPLMCreateMenu("PAP3", mainMenuId, g_pap3ItemIndex, pap3MenuCallback, nullptr);

    g_pap3LabelsIndex = XPLMAppendMenuItem(g_pap3MenuId, "Display LCD Labels", (void *)"toggle_labels", 0);
...
ProductPAP3MCP::ProductPAP3MCP(HIDDeviceHandle hidDevice, uint16_t vendorId, uint16_t productId, std::string vendorName, std::string productName) : USBDevice(hidDevice, vendorId, productId, vendorName, productName) {
    profile = nullptr;
    lastUpdateCycle = 0;
    ...
    debug("Initialized ProductPAP3MCP (VID=0x%04X PID=0x%04X)", vendorId, productId);
    connect();
}
...
bool ProductPAP3MCP::connect() {
    ...
    if (!_inputFLRegistered) {
        ...
        debug("Input FlightLoop registered (per-frame HID polling)");
    }
    ...
    debug("Connected: VID=0x%04X PID=0x%04X (%s %s)", vendorId, productId, vendorName.c_str(), productName.c_str());
    ...
    if (!g_pap3DatarefFLRegistered) {
        ...
        debug("FlightLoop registered for Dataref::update() @ every frame");
    }
    return true;
}
...
void ProductPAP3MCP::disconnect() {
    ...
    if (profile) {
        ...
        debug("Deleting profile: %s", profile->name());
        delete profile;
        profile = nullptr;
    }
...
void ProductPAP3MCP::decodeButtons(const uint8_t *r, int len, const uint8_t *prev) {
    ...
    for (const auto &d : kBtnDefs) {
        ...
        debug("BTN %s: %s", d.name, pressed ? "DOWN" : "UP");
...
void ProductPAP3MCP::decodeSwitches(const uint8_t *r, int len) {
    ...
    for (const auto &s : sw) {
        ...
        if (current != desired) {
            ...
            debug("SW %s: bit %s -> %s = %d", s.name, bitSet ? "SET" : "CLR", s.datarefName, desired);
        }
    }
}
...
void ProductPAP3MCP::decodeEncoders(const uint8_t *r, int len, const uint8_t *prev) {
    ...
    for (const auto &e : kEncDefs) {
        ...
        debug("ENC %s: delta=%d (pos 0x%02X -> 0x%02X)", e.name, d, was, now);
...
void ProductPAP3MCP::initializeDisplays() {
    ...
    debug("initializeDisplays(): LCD init sent (write=%s)", ok ? "ok" : "FAIL");
}
...
void ProductPAP3MCP::setProfileForCurrentAircraft() {
    ...
    if (ZiboMcpProfile::IsEligible()) {
        ...
        debug("Using profile: %s (Zibo detected)", profile->name());
    }
    else {
        debug("No eligible MCP profile found; LEDs/LCD will remain idle");
    }
}
...
void ProductPAP3MCP::monitorDatarefs() {
    ...
    debug("monitorDatarefs(): touched %zu LED datarefs", profile->ledDatarefs().size());
}
...
void ProductPAP3MCP::sendLedRaw(uint8_t ledId, bool on) {
    ...
    debug("LED 0x%02X <- %s (write=%s)", ledId, on ? "ON" : "OFF", ok ? "ok" : "FAIL");
}
...
void ProductPAP3MCP::registerLedMonitors() {
    ...
    debug("Registering %zu LED monitors", bindings.size());
    ...
    for (const auto &b : bindings) {
        ...
        drm->monitorExistingDataref<int>(ref.c_str(), [this, hwLedId, rule = b.rule, name = ref, logical](int v) {
            bool on = false;
            switch (rule) {
                case McpLedRule::GT0:
                    on = (v > 0);
            ...
            if (it == _lastLedValue.end() || it->second != desired) {
                debug("LED change (monitor): DR='%s' val=%d -> LED 0x%02X %s", name.c_str(), v, hwLedId, on ? "ON" : "OFF");
        ...
        if (drm->exists(ref.c_str())) {
            ...
            auto it = _lastLedValue.find(hwLedId);
            if (it == _lastLedValue.end() || it->second != desired) {
                debug("LED initial sync: DR='%s' val=%d -> LED 0x%02X %s",
            ...
            debug("WARN: dataref not found at init: '%s'", ref.c_str());
        }
    }
}
...
void ProductPAP3MCP::unregisterLedMonitors() {
    ...
    debug("Unregistered %zu LED monitors", _ledMonitoredRefs.size());
    _ledMonitoredRefs.clear();
}
...
float ProductPAP3MCP::PAP3_ATSolenoidOnFL(float, float, int, void *inRefcon)
{
    ...
    if (!self->_apHasPower) {
        ...
        debug("AT solenoid ON delayed: no A/P power");
        return 0.20f; // 200 ms
    }
    ...
    debug("AT solenoid ON sent (delayed)");
...
void ProductPAP3MCP::pulseATSolenoid() {
    ...
    debug("AT solenoid OFF sent, ON scheduled in %.0f ms", delaySeconds * 1000.0f);
}
...
void ProductPAP3MCP::registerDisplayMonitors() {
    ...
    for (const auto &b : binds) {
        ...
        if (b.type == McpRefType::Float) {
            drm->monitorExistingDataref<float>(b.datarefName.c_str(), [this, item = b.item](float v) {
                ...
                if (!st.hasFlt || std::fabs(st.lastFlt - v) > 1e-3f) {
                    ...
                    if (item == McpDisplayItem::SPD) {
                        ...
                        if (spdMachNow != _lastSpdMach) {
                            ...
                            debug("SPD mode changed -> %s (re-read SPD + force LCD)", spdMachNow ? "MACH" : "IAS");
                    ...
                    switch (item) {
                        case McpDisplayItem::SPD:
                            debug("MCP SPD change: %.3f", v);
                            break;
                        case McpDisplayItem::VVI:
                            debug("MCP VVI change: %.1f fpm", v);
                            break;
                        default:
                            debug("MCP (float) item=%d change: %.3f", (int)item, v);
                            break;
                    }
            ...
            drm->monitorExistingDataref<int>(b.datarefName.c_str(), [this, item = b.item](int v) {
                auto &st = _displayCache[item];
                if (!st.hasInt || st.lastInt != v) {
                    ...
                    switch (item) {
                        ...
                        case McpDisplayItem::SPD_DIGIT_A:
                            break;
                        default:
                            debug("MCP (int) item=%d change: %d", (int)item, v);
                            break;
                    }
                    onDisplayChanged();
                }
    ...
    debug("Registered %zu MCP display monitors", binds.size());
}
...
void ProductPAP3MCP::unregisterDisplayMonitors() {
    ...
    _displayMonitoredRefs.clear();
    _displayCache.clear();
    debug("Unregistered MCP display monitors");
}
...
void ProductPAP3MCP::forceLcdRefresh() {
    ...
    const auto payload = buildLcdPayloadFromCache();
    sendLcdTransaction(payload);
    debug("LCD refresh forced (labels=%s)", ProductPAP3MCP::getShowLcdLabels() ? "ON" : "OFF");
}
...
void ProductPAP3MCP::refreshSpdFromDatarefAndPush() {
    ...
    if (_spdRefName.empty()) {
        debug("refreshSpdFromDatarefAndPush(): SPD ref not found in bindings");
...
void ProductPAP3MCP::refreshVviFromDatarefAndPush() {
    ...
    if (_vviRefName.empty()) {
        debug("refreshVviFromDatarefAndPush(): VVI ref not found in bindings");
        forceLcdRefresh(); // best effort
        return;
    }
    ...
    debug("VVI re-read after VS ON: %.1f (forcing LCD)", fresh);
    forceLcdRefresh();
}
