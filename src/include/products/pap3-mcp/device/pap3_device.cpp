...
namespace pap3::device {
    ...
    */
    ...
    void PAP3Device::applyState(const pap3::aircraft::State &st) {
        ...
        if (out.changed[0]) {
            setDimming(0, out.raw[0]); // Backlight
        }
        if (out.changed[1]) {
            setDimming(1, out.raw[1]); // LCD
        }
        if (out.changed[2]) {
            setDimming(2, out.raw[2]); // LEDs
        }
    ...
    void PAP3Device::runStartupSequence() {
        ...
        // Apply computed dimming (first frame → all changed)
        {
            ...
            if (out.changed[0]) {
                qSetDimming(0, out.raw[0]);
            }
            if (out.changed[1]) {
                qSetDimming(1, out.raw[1]);
            }
            if (out.changed[2]) {
                qSetDimming(2, out.raw[2]);
            }
        }
    ...
    void StartPap3Demo(pap3::device::PAP3Device *dev) {
        ...
        std::vector<std::uint8_t> ledIds = {
            0x03, 0x04, 0x06, 0x05, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0E, 0x0D, 0x0F, 0x10};

        if (!g_demo) {
            g_demo = new pap3::device::PAP3Demo(dev);
        }
    ...
    void PAP3Device::setupInputCallbacks() {
        ...
        _inputs.onEncoders = [this](const Inputs::EncodersFrame &e) {
            ...
            for (const auto &it : items) {
                ...
                if (_profile) {
                    _profile->onEncoderDelta(it.off, it.d);
                }
            }
        };
        ...
        _inputs.onSwitchEdges = [this](std::uint8_t off, std::uint8_t changedSet, std::uint8_t changedClr) {
            auto forward = [this, off](uint8_t bit, bool pressed) {
                if (_profile) {
                    _profile->onButton(off, bit, pressed);
                }
            };
    ...
    void PAP3Device::updatePower() {
        ...
        const auto out = _illum.compute();
        if (out.changed[0]) {
            qSetDimming(0, out.raw[0]); // Backlight
        }
        if (out.changed[1]) {
            qSetDimming(1, out.raw[1]); // LCD
        }
        if (out.changed[2]) {
            qSetDimming(2, out.raw[2]); // LEDs
        }
