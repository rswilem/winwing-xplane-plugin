...
std::vector<std::uint8_t> PAP3Demo::buildLcdPayloadForWindow(int startCol) const {
    ...
    struct SegGroup { uint8_t g,f,e,d,c,b,a; };
    ...
    auto segsOf = [&](SegGroupId id)->const SegGroup&{
        switch(id){
            case SegGroupId::G1: return G1;
            case SegGroupId::G2: return G2;
            case SegGroupId::G3: return G3;
            default:             return G4;
        }
    };
    ...
    for (int i=0;i<_lineLen;++i) {
        ...
        if (pat & 0b00000010) orAbs(grp.a, slot.mask); // A
        if (pat & 0b00000100) orAbs(grp.b, slot.mask); // B
        if (pat & 0b00001000) orAbs(grp.c, slot.mask); // C
        if (pat & 0b00010000) orAbs(grp.d, slot.mask); // D
        if (pat & 0b00100000) orAbs(grp.e, slot.mask); // E
        if (pat & 0b01000000) orAbs(grp.f, slot.mask); // F
        if (pat & 0b10000000) orAbs(grp.g, slot.mask); // G
    }
...
uint8_t PAP3Demo::segMaskForChar(char c) {
    ...
    switch (c) {
        case 'H': case 'h': return 0b11001110; // H ~ b c e f g
        case 'E': case 'e': return 0b11110010; // E ~ a d e f g
        case 'L': case 'l': return 0b00111000; // L ~ d e f
        case 'O': case 'o': return 0b01111110; // O ~ a b c d e f (g off)
...
void PAP3Demo::ledStepChase() {
    ...
    if (prevI < _ledIds.size()) _dev->setLed(_ledIds[prevI], false);
    if (prevJ < _ledIds.size()) _dev->setLed(_ledIds[prevJ], false);
